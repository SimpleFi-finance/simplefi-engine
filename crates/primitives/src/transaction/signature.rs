...
use bytes::Buf;
use sip_codecs::{derive_arbitrary, Compact};
...
impl From<Value> for Signature {
    fn from(value: Value) -> Self {
        let r = U256::from_str(value["r"].as_str().unwrap()).unwrap();
        let s = U256::from_str(value["s"].as_str().unwrap()).unwrap();
        let odd_y_parity = value["odd_y_parity"].as_bool().unwrap_or(false);
...
impl Signature {
    ...
    pub(crate) fn payload_len_with_eip155_chain_id(&self, chain_id: u64) -> usize {
    ...
    pub fn encode(&self, out: &mut dyn rlp::BufMut) {
...
mod tests {
    ...
    fn test_payload_len_with_eip155_chain_id() {
        ...
        assert_eq!(3, signature.payload_len_with_eip155_chain_id(0));
        assert_eq!(3, signature.payload_len_with_eip155_chain_id(1));
        assert_eq!(4, signature.payload_len_with_eip155_chain_id(47));
    }

    #[test]
    fn test_v() {
        let signature = Signature { r: U256::default(), s: U256::default(), odd_y_parity: false };
        assert_eq!(27, signature.v(0));
        assert_eq!(37, signature.v(1));

        let signature = Signature { r: U256::default(), s: U256::default(), odd_y_parity: true };
        assert_eq!(28, signature.v(0));
        assert_eq!(38, signature.v(1));
    }
    ...
    fn test_encode_and_decode_with_eip155_chain_id() {
        ...
        signature.encode_with_eip155_chain_id(&mut encoded, 0);
        assert_eq!(encoded.len(), signature.payload_len_with_eip155_chain_id(0));
        let (decoded, chain_id) = Signature::decode_with_eip155_chain_id(&mut &*encoded).unwrap();
        assert_eq!(signature, decoded);
        assert_eq!(None, chain_id);

        let mut encoded = BytesMut::new();
        signature.encode_with_eip155_chain_id(&mut encoded, 1);
        assert_eq!(encoded.len(), signature.payload_len_with_eip155_chain_id(1));
        let (decoded, chain_id) = Signature::decode_with_eip155_chain_id(&mut &*encoded).unwrap();
        assert_eq!(signature, decoded);
