...
pub fn merge_bal_vecs(
    vec_a: &Vec<AddressBalance>,
    vec_b: &Vec<AddressBalance>,
) -> Vec<AddressBalance> {
    let mut new_vec: Vec<AddressBalance> = vec![];
    ...
    vec_a.iter().for_each(|val| {
        let matching_token = new_vec
            .iter()
            .position(|x| x.address == val.address);

        match matching_token {
            Some(x) => new_vec[x].balance = new_vec[x].balance + val.balance,
            _ => new_vec.push(val.clone()),
        }
    });
