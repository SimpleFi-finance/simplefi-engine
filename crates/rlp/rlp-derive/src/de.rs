...
pub(crate) fn impl_decodable(ast: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    let impl_block = quote! {
        impl #impl_generics rlp::Decodable for #name #ty_generics #where_clause {
            fn decode(mut buf: &mut &[u8]) -> Result<Self, rlp::DecodeError> {
                let b = &mut &**buf;
                let rlp_head = rlp::Header::decode(b)?;
    ...
    let impl_block = quote! {
        impl #impl_generics rlp::Decodable for #name #ty_generics #where_clause {
            fn decode(buf: &mut &[u8]) -> Result<Self, rlp::DecodeError> {
                Ok(Self(rlp::Decodable::decode(buf)?))
            }
        }
    };
...
fn decodable_field(index: usize, field: &syn::Field, is_opt: bool) -> TokenStream {
    ...
    if attributes_include(&field.attrs, "default") {
        ...
        quote! { #ident: rlp::Decodable::decode(b)?, }
    }
}
