...
pub(crate) fn impl_encodable(ast: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    let impl_block = quote! {
        trait E {
            fn rlp_header(&self) -> rlp::Header;
        }

        impl #impl_generics E for #name #ty_generics #where_clause {
            fn rlp_header(&self) -> rlp::Header {
                let mut rlp_head = rlp::Header { list: true, payload_length: 0 };
                #(#length_stmts)*
                rlp_head
            }
        }
        ...
        impl #impl_generics rlp::Encodable for #name #ty_generics #where_clause {
            fn length(&self) -> usize {
                ...
                return rlp::length_of_length(rlp_head.payload_length) + rlp_head.payload_length;
            }
...
pub(crate) fn impl_max_encoded_len(ast: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    let impl_block = quote! {
        unsafe impl rlp::MaxEncodedLen<{ rlp::const_add(rlp::length_of_length(#(#stmts)*), #(#stmts)*) }> for #name {}
        unsafe impl rlp::MaxEncodedLenAssoc for #name {
            const LEN: usize = { rlp::const_add(rlp::length_of_length(#(#stmts)*), { #(#stmts)* }) };
        }
    };
...
fn encodable_length<'a>(
    ...
    if is_opt {
        ...
        quote! { rlp_head.payload_length += &self.#ident.as_ref().map(|val| rlp::Encodable::length(val)).unwrap_or(#default); }
    } else {
        quote! { rlp_head.payload_length += rlp::Encodable::length(&self.#ident); }
    }
}
...
fn encodable_max_length(index: usize, field: &syn::Field) -> TokenStream {
    ...
    if index == 0 {
        quote! { <#fieldtype as rlp::MaxEncodedLenAssoc>::LEN }
    } else {
        quote! { + <#fieldtype as rlp::MaxEncodedLenAssoc>::LEN }
    }
}
...
fn encodable_field<'a>(
    ...
    if is_opt {
        ...
        quote! { rlp::Encodable::encode(&self.#ident, out); }
    }
}
