use crate::{
    ...
    utils::{
        balance_strings::{add_bal_string_to_bal_string, merge_bal_string_vecs},
        big_number::add_big_from_strs,
    },
};
...
pub fn aggregate_volumetrics(
    ...
    let x = volumetrics[1..]
        ...
        .fold(vec![initial], |mut acc, volumetric| {
            ...
            if same_period {
                current.mint = merge_bal_string_vecs(&current.mint, &volumetric.mint);
                current.withdrawal =
                    merge_bal_string_vecs(&current.withdrawal, &volumetric.withdrawal);
                current.swaps_in = merge_bal_string_vecs(&current.swaps_in, &volumetric.swaps_in);
                current.swaps_out =
                    merge_bal_string_vecs(&current.swaps_out, &volumetric.swaps_out);
                current.transfer = add_big_from_strs(&current.transfer, &volumetric.transfer);
...
pub fn create_period_volume(
    ...
    let x = volumetrics[1..]
        ...
        .fold(initial, |mut acc, volumetric| {
            acc.mint = merge_bal_string_vecs(&acc.mint, &volumetric.mint);
            acc.withdrawal = merge_bal_string_vecs(&acc.withdrawal, &volumetric.withdrawal);
            acc.swaps_in = merge_bal_string_vecs(&acc.swaps_in, &volumetric.swaps_in);
            acc.swaps_out = merge_bal_string_vecs(&acc.swaps_out, &volumetric.swaps_out);
            acc.transfer = add_big_from_strs(&acc.transfer, &volumetric.transfer);
            acc.withdrawal = vec![
