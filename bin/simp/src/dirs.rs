//! simp  data directories.
use crate::utils::parse_path;
use simp_primitives::Chain;
use std::{
    env::VarError,
...
/// Returns the path to the simp data directory.
///
/// Refer to [dirs_next::data_dir] for cross-platform behavior.
pub fn data_dir() -> Option<PathBuf> {
    dirs_next::data_dir().map(|root| root.join("simp"))
}

/// Returns the path to the simp database.
///
/// Refer to [dirs_next::data_dir] for cross-platform behavior.
...
/// Returns the path to the simp configuration directory.
///
/// Refer to [dirs_next::config_dir] for cross-platform behavior.
pub fn config_dir() -> Option<PathBuf> {
    dirs_next::config_dir().map(|root| root.join("simp"))
}

/// Returns the path to the simp cache directory.
///
/// Refer to [dirs_next::cache_dir] for cross-platform behavior.
pub fn cache_dir() -> Option<PathBuf> {
    dirs_next::cache_dir().map(|root| root.join("simp"))
}

/// Returns the path to the simp logs directory.
///
/// Refer to [dirs_next::cache_dir] for cross-platform behavior.
...
/// Returns the path to the simp data dir.
///
...
///
/// ```
/// use simp::dirs::{PlatformPath, DataDirPath};
/// use std::str::FromStr;
///
...
/// Wrapper type around PlatformPath that includes a `Chain`, used for separating simp data for
/// different networks.
///
...
impl<D> ChainPath<D> {
    ...
    /// Returns the path to the simp p2p secret key for this chain.
...
mod tests {
    ...
    fn test_maybe_data_dir_path() {
        ...
        assert!(path.as_ref().ends_with("simp/mainnet"), "{:?}", path);

        let db_path = path.db_path();
        assert!(db_path.ends_with("simp/mainnet/db"), "{:?}", db_path);
