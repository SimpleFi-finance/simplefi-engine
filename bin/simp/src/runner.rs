...
use futures::pin_mut;
use simp_tasks::{TaskExecutor, TaskManager};
...
impl CliRunner {
    ...
    where
        ...
        E: Send + Sync + From<std::io::Error> + From<simp_tasks::PanickedTaskError> + 'static,
    {
        ...
        // warn!(target: "simp::cli", "Received shutdown signal, waiting up to 30 seconds for
...
pub fn tokio_runtime() -> Result<tokio::runtime::Runtime, std::io::Error> {
    tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        // increase stack size, mostly for RPC calls that use the evm: <https://github.com/paradigmxyz/simp/issues/3056> and  <https://github.com/bluealloy/revm/issues/305>
...
where
    F: Future<Output = Result<(), E>>,
    E: Send + Sync + From<simp_tasks::PanickedTaskError> + 'static,
{
    {
        pin_mut!(fut);
        tokio::select! {
    ...
    E: Send + Sync + 'static + From<std::io::Error>,
{
    ...
    #[cfg(unix)]
    {
        ...
        tokio::select! {
            _ = ctrl_c => {
                trace!(target: "simp::cli",  "Received ctrl-c");
            },
            _ = sigterm => {
                trace!(target: "simp::cli",  "Received SIGTERM");
            },
            res = fut => res?,
        }
    }
